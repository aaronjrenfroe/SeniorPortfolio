  ...
  try {
    ...
    posts.forEach((post) => {
      ...
      post.Events.forEach((evID) => {
        ...
        if(event){ // If no event it means user is not attending this event event
          ...
          // Visible Date
          if(postClone.Date_visible){
            let eventStartMoment = moment(event.BeginDateTime,'YYYY-M-DTHH:mm:ss');
            
            // add offset value to Date_visible
            eventStartMoment.add(postClone.Date_visible, 'days');
            postClone.visible_Date = eventStartMoment.format('YYYY-M-DT00:06:00');
          }else{
            postClone.Date_visible = '0';
            postClone.visible_Date = moment(postClone.Timestamp).format('YYYY-M-DT00:06:00');
          }
    ...
    unBoundPosts.forEach(post => {
      ...
      delete postClone.__v;
        // Visible Date
      if(postClone.Date_visible){
        postClone.visible_Date = moment(post.Expire_Date).format('YYYY-M-DT00:06:00');
      }else{
        postClone.Date_visible="0";
        postClone.visible_Date = moment(post.Timestamp).format('YYYY-M-DTHH:m:s');
      }
    ...
    postbundle.forEach(post => {
      ...
      post.Date_visible = post.Date_visible + "";
...
router.get('/api/post/update', (rew, res) => {
  try {
    Post.find((error, posts) => {
    if (error) {
      return res.send([]);
    ...
    posts.forEach(post => {
      post.User_ID = "5a62e00bff1f7a00370ee731";
      post.save();
    })
  });
} catch (err) {
...
router.post('/api/posts/update/:id', authenticate,(req, res) => {
  ...
  if(np.offset != op.Date_visible){
    update.Date_visible = np.offset
  }
